document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch(`songs/${fileName}`)
        ...
        .then(songData => {
            ...
            // Převede \n na <br>
            let songText = songData.text.replace(/\n/g, '<br>');

            // Převede [Ami] -> <sup class="inline-chord Ami">Ami</sup>
            songText = songText.replace(
                /\[([^\]]+)\]/g,
                (match, chord) => `<sup class="inline-chord ${chord}">${chord}</sup>`
            );

            // Vloží finální text
            songDetailElement.innerHTML = songText;
