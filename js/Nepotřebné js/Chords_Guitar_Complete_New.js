 ...
 -- Slugify funkce pro CSS třídy, např. A# → Ax, A+ → Ay, DF# → D--Fx, Ami → Am
function get_slugify_chord(chord) {
    chord = chord.replace((minmim)(!aa7aj)gi, 'm');
    chord = chord.replace(#g, 'x').replace(+g, 'y').replace(g, '--');
    return chord;
}
...
function chord_bind_hover() {
    $('.inline-chord').each((idx, el) = {
        $(el).append('span class=chord-tooltipspan');
    });
 ...
 -- Transpozice (zachována kvi funkci, lze přizpůsobit pro jiné nástroje)
(function ($) {
